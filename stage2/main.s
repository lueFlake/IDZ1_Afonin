...
main:
	...
	mov	r14, rsi		# argv = rsi

	mov	rax, r14		# rax = argv
	...
	mov	r13d, eax		# n = genInput()
	jmp	.L3 						# Чтобы не перейти в else(LC2)

.L2: # else
	mov	rax, r14		# (см. предыдущие комментарии, т. к. вызовы повторяются)
	add	rax, 8						#
	mov	rax, QWORD PTR [rax]		# rax = argv[1]
	...
	mov r13d, DWORD PTR -36[rbp]

	mov	eax, r13d		# eax (rax) = n
